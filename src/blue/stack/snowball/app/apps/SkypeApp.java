...
public class SkypeApp extends BaseApp implements NotificationProcessor {
	...
	void initMessageProcessors() {
		addMessageProcessor(new MessageProcessorBuilder()
				.addInputFormat(
						"contentTitle",
						new InputFormatBuilder().setInputTemplateFromPackage(PACKAGE_NAME, "message_new_messages")
								.addInputToken("%1$d", "count").build(this.context))
				.addInputFormat(
				...
				.addOutputFormat(
						"from",
						new OutputFormatBuilder().setOutputTemplateFromResouce(R.string.skype_entry_of_from_unknown)
								.build(this.context))
				.addOutputFormat(
						"message",
						new OutputFormatBuilder()
								.setOutputTemplateFromResouce(R.string.skype_entry_of_message_multiple).build(
										this.context)).build(this.context));
		addMessageProcessor(new MessageProcessorBuilder().addInputFormat("contentTitle",
				new InputFormatBuilder().setInputTemplate("Skype").build(this.context)).build(this.context));
		addMessageProcessor(new MessageProcessorBuilder()
				.addInputFormat(
						"contentTitle",
